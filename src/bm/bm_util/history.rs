...
use super::table_types::{
    new_butterfly_table, new_piece_piece_to_table, new_piece_to_table, Butterfly, PiecePieceTo,
    PieceTo,
};
...
pub struct History {
    ...
    capture: Box<[[PiecePieceTo<i16>; 2]; Color::NUM]>,
...
impl History {
    pub fn new() -> Self {
        Self {
            ...
            capture: Box::new([[new_piece_piece_to_table(0); Color::NUM]; 2]),
    ...
    pub fn get_capture(&self, pos: &Position, make_move: Move) -> i16 {
        let stm = pos.board().side_to_move();
        let (_, nstm_threats) = pos.threats();
        let mv_piece = pos.board().piece_on(make_move.from).unwrap();
        let cap_piece = pos.board().piece_on(make_move.to).unwrap();
        self.capture[stm as usize][nstm_threats.has(make_move.from) as usize][mv_piece as usize]
            [cap_piece as usize][make_move.to as usize]
    }
    ...
    fn get_capture_mut(&mut self, pos: &Position, make_move: Move) -> &mut i16 {
        ...
        &mut self.capture[stm as usize][nstm_threats.has(make_move.from) as usize]
            [mv_piece as usize][cap_piece as usize][make_move.to as usize]
    }
