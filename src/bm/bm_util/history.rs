...
pub struct History {
    ...
    captured: Box<[PieceTo<i16>; Color::NUM]>,
...
impl History {
    pub fn new() -> Self {
        Self {
            ...
            captured: Box::new([new_piece_to_table(0); Color::NUM]),
    ...
    pub fn get_captured(&self, pos: &Position, make_move: Move) -> i16 {
        let stm = pos.board().side_to_move();
        let captured = pos.board().piece_on(make_move.to).unwrap();
        self.captured[stm as usize][captured as usize][make_move.to as usize]
    }

    fn get_captured_mut(&mut self, pos: &Position, make_move: Move) -> &mut i16 {
        let stm = pos.board().side_to_move();
        let captured = pos.board().piece_on(make_move.to).unwrap();
        &mut self.captured[stm as usize][captured as usize][make_move.to as usize]
    }
    ...
    pub fn update_history(
        ...
        if !is_capture {
            ...
            bonus(self.get_captured_mut(pos, make_move), amt);
        }
        for &failed_move in captures {
            malus(self.get_capture_mut(pos, failed_move), amt);
            malus(self.get_captured_mut(pos, failed_move), amt);
        }
    }
