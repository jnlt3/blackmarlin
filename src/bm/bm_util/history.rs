...
use super::table_types::{
    new_butterfly_table, new_piece_to_table, new_sq_table, Butterfly, PieceTo, Sq,
};
...
#[derive(Debug, Clone)]
pub struct History {
    from: Box<[[Sq<i16>; 2]; Color::NUM]>,
    to: Box<[[Sq<i16>; 2]; Color::NUM]>,
...
impl History {
    pub fn new() -> Self {
        Self {
            from: Box::new([[new_sq_table(0); 2]; Color::NUM]),
            to: Box::new([[new_sq_table(0); 2]; Color::NUM]),
    ...
    pub fn get_from(&self, pos: &Position, make_move: Move) -> i16 {
        ...
        self.from[stm as usize][nstm_threats.has(make_move.from) as usize][make_move.from as usize]
    }

    fn get_from_mut(&mut self, pos: &Position, make_move: Move) -> &mut i16 {
        let stm = pos.board().side_to_move();
        let (_, nstm_threats) = pos.threats();
        &mut self.from[stm as usize][nstm_threats.has(make_move.from) as usize]
            [make_move.from as usize]
    }

    pub fn get_to(&self, pos: &Position, make_move: Move) -> i16 {
        let stm = pos.board().side_to_move();
        let (_, nstm_threats) = pos.threats();
        self.to[stm as usize][nstm_threats.has(make_move.from) as usize][make_move.to as usize]
    }

    fn get_to_mut(&mut self, pos: &Position, make_move: Move) -> &mut i16 {
        let stm = pos.board().side_to_move();
        let (_, nstm_threats) = pos.threats();
        &mut self.to[stm as usize][nstm_threats.has(make_move.from) as usize][make_move.to as usize]
    }
