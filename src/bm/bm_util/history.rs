...
#[derive(Debug, Clone)]
pub struct History {
    quiet: Box<[[PieceTo<i16>; u16::MAX as usize + 1]; Color::NUM]>,
    capture: Box<[[PieceTo<i16>; u16::MAX as usize + 1]; Color::NUM]>,
...
impl History {
    pub fn new() -> Self {
        Self {
            quiet: Box::new([[new_piece_to_table(0); u16::MAX as usize + 1]; Color::NUM]),
            capture: Box::new([[new_piece_to_table(0); u16::MAX as usize + 1]; Color::NUM]),
    ...
    pub fn get_quiet(&self, pos: &Position, make_move: Move) -> i16 {
        ...
        self.quiet[stm as usize][pos.threat_hash() as usize][current_piece as usize]
    ...
    fn get_quiet_mut(&mut self, pos: &Position, make_move: Move) -> &mut i16 {
        let stm = pos.board().side_to_move();
        let current_piece = pos.board().piece_on(make_move.from).unwrap();
        &mut self.quiet[stm as usize][pos.threat_hash() as usize][current_piece as usize]
            [make_move.to as usize]
    }
    ...
    pub fn get_capture(&self, pos: &Position, make_move: Move) -> i16 {
        let stm = pos.board().side_to_move();
        let current_piece = pos.board().piece_on(make_move.from).unwrap();
        self.capture[stm as usize][pos.threat_hash() as usize][current_piece as usize]
            [make_move.to as usize]
    }
    ...
    fn get_capture_mut(&mut self, pos: &Position, make_move: Move) -> &mut i16 {
        let stm = pos.board().side_to_move();
        let current_piece = pos.board().piece_on(make_move.from).unwrap();
        &mut self.capture[stm as usize][pos.threat_hash() as usize][current_piece as usize]
            [make_move.to as usize]
    }
