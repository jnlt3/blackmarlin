...
pub enum EntryType {
    Missing,
    Entry { bounds: Bounds, low_eval: bool },
}
...
impl EntryType {
    fn to_u16(self) -> u16 {
        match self {
            EntryType::Missing => 0,
            EntryType::Entry { bounds, low_eval } => {
                let bound = match bounds {
                    Bounds::LowerBound => 0,
                    Bounds::Exact => 1,
                    Bounds::UpperBound => 2,
                };
                1 + (bound + 3 * low_eval as u16)
            }
        }
    }
...
impl Analysis {
    fn from_raw(bits: u64) -> Option<Self> {
        ...
        match entry {
            ...
            EntryType::Entry { bounds, low_eval } => Some(Self {
                depth: depth as u32,
                low_eval,
                bounds,
