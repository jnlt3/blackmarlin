...
pub enum EntryType {
    Missing,
    Entry { bounds: Bounds, nmp_fail: bool },
}
...
impl EntryType {
    fn to_u16(self) -> u16 {
        match self {
            EntryType::Missing => 0,
            EntryType::Entry { bounds, nmp_fail } => {
                let bound = match bounds {
                    Bounds::LowerBound => 0,
                    Bounds::Exact => 1,
                    Bounds::UpperBound => 2,
                };
                1 + (bound + 3 * nmp_fail as u16)
            }
        }
    }
...
impl Analysis {
    fn from_raw(bits: u64) -> Option<Self> {
        ...
        match entry {
            ...
            EntryType::Entry { bounds, nmp_fail } => Some(Self {
                depth: depth as u32,
                nmp_fail,
                bounds,
