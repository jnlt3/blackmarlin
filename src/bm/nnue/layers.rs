...
const INTERMEDIATE_SHIFT: i32 = 4;
...
impl<const INPUT: usize, const OUTPUT: usize> Dense<INPUT, OUTPUT> {
    pub fn new(
        weights: Arc<Aligned<[[i8; INPUT]; OUTPUT]>>,
        mut bias: Aligned<[i16; OUTPUT]>,
    ) -> Self {
        bias.0.iter_mut().for_each(|b| *b >>= INTERMEDIATE_SHIFT);
        Self { weights, bias }
    }
    ...
    unsafe fn sum_i16(x: std::arch::x86_64::__m128i) -> i16 {
    ...
    pub fn ff(&self, inputs: &[u8; INPUT]) -> [i16; OUTPUT] {
        use std::arch::x86_64::{
            ...
            _mm256_maddubs_epi16, _mm256_setzero_si256, _mm256_srai_epi16, _mm_add_epi16,
        };
        ...
        for (out, weights) in out.iter_mut().zip(&self.weights.0) {
            unsafe {
                ...
                for (inputs, weights) in inputs.chunks(CHUNKS).zip(weights.chunks(CHUNKS)) {
                    ...
                    let out_shifted = _mm256_srai_epi16::<INTERMEDIATE_SHIFT>(out);
                    sums = _mm256_add_epi16(sums, out_shifted);
                }
...
pub struct Out<const INPUT: usize, const OUTPUT: usize> {
    weights: Arc<Aligned<[[i16; INPUT]; OUTPUT]>>,
