...
impl Nnue {
    pub fn new() -> Self {
        ...
        let out = Arc::new(*include::dense_from_bytes_i8::<i16, MID_1, OUTPUT>(bytes));
        ...
        let out_layer = Out::new(out, out_bias);
    ...
    pub fn feed_forward(&mut self, stm: Color) -> i16 {
        ...
        layers::sq_clipped_relu(*nstm.get(), &mut incr[MID_0..]);

        let mut hidden_buff = [0; MID_1];
        let hidden = self.hidden_layer.ff(&incr);
        layers::scaled_clipped_relu(hidden, &mut hidden_buff);
        layers::out(self.out_layer.ff(&hidden_buff)[0] as i32)
    }
}
