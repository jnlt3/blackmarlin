...
pub struct Psqt<'a, const INPUT: usize, const OUTPUT: usize> {
    weights: &'a [[i16; OUTPUT]; INPUT],
    out: [i32; OUTPUT],
}

impl<'a, const INPUT: usize, const OUTPUT: usize> Psqt<'a, INPUT, OUTPUT> {
    pub fn new(weights: &'a [[i16; OUTPUT]; INPUT]) -> Self {
        Self {
            weights,
            out: [0; OUTPUT],
        }
...
impl<'a, const INPUT: usize, const OUTPUT: usize> Dense<'a, INPUT, OUTPUT> {
    ...
    pub fn ff_sym(
        ...
        let mut out_0 = [0; OUTPUT];
        let mut out_1 = [0; OUTPUT];
        for ((&w_input, &b_input), weights) in
            w_inputs.iter().zip(b_inputs.iter()).zip(&*self.weights)
        {
            for ((out_0, out_1), &weight) in out_0[bucket..bucket + 1]
                .iter_mut()
                .zip(&mut out_1[bucket..bucket + 1])
                .zip(weights[bucket..bucket + 1].iter())
            {
                *out_0 += weight as i32 * w_input as i32;
                *out_1 += weight as i32 * b_input as i32;
            }
        }
        let mut out = [0; OUTPUT];
        out[bucket] = (out_0[bucket] - out_1[bucket]) / 2;
        out
    }
}
...
pub fn out(x: i32) -> i16 {
    (x * UNITS as i32 / (SCALE * SCALE) as i32) as i16
}
