...
fn do_nmp<Search: SearchType>(
    ...
    let rev_piece_cnt = 32 - piece_cnt;
    Search::NM
        && depth > 4
        && eval - (rev_piece_cnt * 2) as i16 >= beta
    ...
    */
    ...
    if !Search::PV && !in_check && skip_move.is_none() {
        ...
        */
        ...
        let piece_cnt = pos.board().occupied().popcnt();
        if do_nmp::<Search>(pos.board(), depth, eval.raw(), beta.raw(), piece_cnt)
            && pos.null_move()
        {
