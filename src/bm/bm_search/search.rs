    ...
    while let Some(make_move) = move_gen.next(local_context.get_h_table().borrow()) {
        ...
        if moves_seen == 0 {
            ...
            */
            ...
            let in_check_e3 = in_check as i32 * 200;
            let tactical_e3 = (is_capture || is_promotion) as i32 * 240;

            let scale_e5 = eval.raw() as i32 * -15
                + h_score as i32 * 89
                + see as i32 * 127
                + alpha.raw() as i32 * 4
                + 200;
            let depth_e3 = depth as i32 * 155;

            let prune_value = in_check_e3 + tactical_e3 + scale_e5 / 100 + depth_e3;
            if !Search::PV && eval.raw().abs() < 1000 && depth <= 5 && prune_value < 0 {
                position.unmake_move();
                continue;
            }
