...
pub fn compare_see(board: &Board, make_move: Move, cmp: i16) -> bool {
    ...
    match make_move.promotion {
        Some(promo) => {
            piece = Some(promo);
            gain += piece_pts(promo) - piece_pts(Piece::Pawn);
        }
        None => {}
    };
    ...
    'outer: for i in 1..16 {
        ...
        for &attacker in &Piece::ALL {
            ...
            if let Some(sq) = potential.next_square() {
                ...
                let new_piece = match attacker {
                    Piece::Pawn => match target.rank() {
                        Rank::First | Rank::Eighth => Piece::Queen,
                        _ => Piece::Pawn,
                    },
                    _ => attacker,
                };

                let promo_extra = piece_pts(new_piece) - piece_pts(attacker);
                let move_value = piece.map_or(0, piece_pts) + promo_extra;
                match start_stm {
...
pub fn calculate_see(board: &Board, make_move: Move) -> i16 {
    ...
    let mut move_gain = match is_quiet {
        true => 0,
        false => match board.piece_on(target) {
            Some(piece) => piece_pts(piece),
            None => 0,
    ...
    match make_move.promotion {
        Some(promo) => {
            piece = Some(promo);
            move_gain += piece_pts(promo) - piece_pts(Piece::Pawn);
        }
        None => {}
    };
    if piece == Some(Piece::King) {
    ...
    'outer: for i in 1..16 {
        for &attacker in &Piece::ALL {
            ...
            if let Some(sq) = potential.next_square() {
                ...
                let promo_extra = piece_pts(new_piece) - piece_pts(attacker);
                gains[i] = piece.map_or(0, piece_pts) + promo_extra - gains[i - 1];
                piece = Some(new_piece);
                stm = !stm;
                continue 'outer;
            }
