...
impl TimeManager {
    pub fn deepen(
        ...
        move_stability = match Some(mv) == *prev_move {
            true => (move_stability + 1).min(13),
            false => 0,
        };
        *prev_move = Some(mv);
        self.move_stability.store(move_stability, Ordering::Relaxed);
        let move_stability_factor = (41 - move_stability) as f32 * 0.024;
        let node_factor = (1.0 - move_nodes as f32 / nodes as f32) * 3.23 + 0.52;
        let eval_factor = (prev_eval - eval).clamp(18, 20) as f32 * 0.092;
